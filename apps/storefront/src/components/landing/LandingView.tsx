...
export default function LandingView({ locale }: { locale: Locale }) {
  ...
  const btnPrimaryBase = `group items-center justify-center gap-2 bg-primary-600 text-white font-semibold rounded-xl shadow-md shadow-primary-600/20 transition-all duration-200 hover:bg-primary-700 hover:shadow-lg hover:shadow-primary-600/30 motion-safe:hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98] ${focusRing}`;
  const btnPrimary = `inline-flex ${btnPrimaryBase}`;
  // Aksent (amber): faqat eng muhim harakat, "Do'kon ochish". Ustidagi matn to'q, hover/active'da ham
  const btnAccent = `group inline-flex items-center justify-center gap-2 bg-accent-500 text-gray-900 font-semibold rounded-xl shadow-md shadow-accent-500/25 transition-all duration-200 hover:bg-accent-400 hover:shadow-lg hover:shadow-accent-500/35 active:bg-accent-600 motion-safe:hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98] ${focusRing}`;
  const btnSecondaryBase = `items-center justify-center gap-2 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-950 text-gray-700 dark:text-gray-200 font-medium rounded-xl transition-all duration-200 hover:border-primary-500 dark:hover:border-primary-500 hover:text-primary-700 dark:hover:text-primary-400 hover:shadow-md motion-safe:hover:-translate-y-0.5 active:translate-y-0 ${focusRing}`;
  const btnSecondary = `inline-flex ${btnSecondaryBase}`;
  const arrow = "transition-transform duration-200 group-hover:translate-x-1";
  const navLink =
    "relative py-1 transition-colors hover:text-gray-900 dark:hover:text-white after:absolute after:left-0 after:-bottom-0.5 after:h-0.5 after:w-full after:origin-left after:scale-x-0 after:rounded-full after:bg-primary-500 after:transition-transform after:duration-200 hover:after:scale-x-100";
  ...
  const mobileLink = `py-2.5 font-medium ${muted} transition-colors hover:text-primary-700 dark:hover:text-primary-400`;
  const footerLink = "transition-colors hover:text-primary-600 dark:hover:text-primary-400";
    ...
    <div className="overflow-x-clip bg-white text-gray-900 dark:bg-gray-950 dark:text-gray-100 transition-colors">
        ...
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between gap-3 lg:grid lg:grid-cols-[1fr_auto_1fr]">
          ...
          <div className="flex items-center gap-2 sm:gap-3 lg:justify-self-end">
            ...
            <a
              ...
              className={`hidden sm:inline-flex text-sm rounded-lg px-4 py-2 whitespace-nowrap ${btnSecondaryBase}`}
            >
              {c.nav.open}
            </a>
            <button
        ...
        {menuOpen && (
          <div className="lp-drop lg:hidden border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-950">
            <nav className="max-w-6xl mx-auto px-4 py-3 flex flex-col text-sm">
              ...
              <a
                href={`${ADMIN_URL}/register`}
                className={`mt-2 mb-1 rounded-lg px-4 py-2.5 ${btnAccent}`}
              >
                {c.nav.open}
              </a>
            </nav>
      ...
      <section className="relative isolate">
        <div aria-hidden="true" className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
          ...
          <div className="lp-float absolute left-1/2 -top-32 h-[480px] w-[820px] max-w-[160vw] -translate-x-1/2 rounded-full bg-primary-400/25 dark:bg-primary-500/10 blur-3xl" />
        </div>

        <div className="max-w-6xl mx-auto px-4 pt-16 md:pt-24 pb-16 md:pb-20 text-center">
          <span
            className="lp-enter inline-flex items-center gap-2 max-w-full text-xs font-semibold tracking-wide uppercase text-primary-700 dark:text-primary-400 bg-primary-50 dark:bg-primary-950/40 border border-primary-100 dark:border-primary-900 rounded-full px-3 py-1.5 mb-6"
            style={enter(0)}
          >
            <span className="relative flex h-2 w-2 shrink-0">
              <span className="absolute inline-flex h-full w-full rounded-full bg-primary-500 opacity-60 motion-safe:animate-ping" />
          ...
          >
            {c.hero.title1}{" "}
            <span className="text-primary-600 dark:text-primary-400 whitespace-nowrap">{c.hero.accent}</span>
          ...
          <div className="lp-enter flex flex-col sm:flex-row gap-3 justify-center mt-8" style={enter(240)}>
            <a
              href={`${ADMIN_URL}/register`}
              className={`w-full sm:w-auto sm:min-w-[240px] px-6 py-3.5 ${btnAccent}`}
            >
              {c.hero.start}
          ...
          >
            {c.hero.bullets.map((item) => (
              <span key={item} className="inline-flex items-center gap-1.5">
                <CheckIcon size={14} className="text-primary-600 dark:text-primary-400" />
                {item}
              </span>
            ))}
          </div>
            ...
            {c.features.items.map((f, i) => {
              ...
              return (
                <div key={f.title} className="lp-reveal" style={delay((i % 3) * 90)}>
                  <div className="group relative h-full overflow-hidden rounded-2xl p-6 bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:border-primary-300 dark:hover:border-primary-700 hover:shadow-xl hover:shadow-primary-900/5 dark:hover:shadow-black/40 motion-safe:hover:-translate-y-1">
                    <div
                      aria-hidden="true"
                      className="pointer-events-none absolute -right-10 -top-10 h-32 w-32 rounded-full bg-primary-400/0 blur-2xl transition-colors duration-500 group-hover:bg-primary-400/20"
                    />
                    <div className="relative w-11 h-11 rounded-xl bg-primary-50 dark:bg-primary-950/40 text-primary-600 dark:text-primary-400 flex items-center justify-center mb-4 transition-all duration-300 group-hover:bg-primary-600 group-hover:text-white group-hover:shadow-lg group-hover:shadow-primary-600/30 motion-safe:group-hover:scale-110">
                      <Icon size={22} />
                    </div>
                    <h3 className="relative font-semibold text-lg transition-colors group-hover:text-primary-700 dark:group-hover:text-primary-400">
                      {f.title}
                    </h3>
          ...
          {c.how.steps.map((s, i) => (
            <div key={s.title} className="lp-reveal" style={delay(i * 110)}>
              <div className="group h-full text-center rounded-2xl p-6 transition-colors duration-300 hover:bg-gray-50 dark:hover:bg-gray-900">
                <div className="relative w-12 h-12 rounded-full bg-primary-600 text-white text-lg font-bold flex items-center justify-center mx-auto mb-4 shadow-lg shadow-primary-600/30 ring-4 ring-white dark:ring-gray-950 transition-transform duration-300 motion-safe:group-hover:scale-110">
                  {i + 1}
                </div>
                <h3 className="font-semibold text-lg transition-colors group-hover:text-primary-700 dark:group-hover:text-primary-400">
                  {s.title}
                </h3>
              ...
              <div key={p.name} className="lp-reveal" style={delay(i * 110)}>
                <div
                  className={`group relative h-full rounded-2xl border p-7 flex flex-col transition-all duration-300 motion-safe:hover:-translate-y-1 ${
                    p.highlight
                      ? "bg-primary-600 border-primary-600 text-white shadow-xl shadow-primary-600/25 hover:shadow-2xl hover:shadow-primary-600/35 md:scale-[1.02]"
                      : "bg-white dark:bg-gray-950 border-gray-200 dark:border-gray-800 hover:border-primary-300 dark:hover:border-primary-700 hover:shadow-xl hover:shadow-gray-900/5 dark:hover:shadow-black/40"
                  }`}
                >
                  {p.highlight && (
                  ...
                  <div className={`text-sm font-medium ${p.highlight ? "text-primary-100" : "text-gray-500 dark:text-gray-400"}`}>
                    {p.tagline}
                  </div>
                  ...
                  <div className="mt-4 mb-6 flex items-baseline gap-2">
                    ...
                    {!p.free && (
                      <span className={`text-sm ${p.highlight ? "text-primary-100" : "text-gray-500 dark:text-gray-400"}`}>
                        {c.pricing.period}
                      </span>
                    )}
                  </div>
                  ...
                  <a
                    ...
                    className={`mt-7 block text-center font-semibold rounded-xl py-3 transition-all duration-200 motion-safe:hover:-translate-y-0.5 active:translate-y-0 hover:shadow-md ${focusRing} ${
                      p.highlight
                        ? "bg-white text-primary-700 hover:bg-primary-50 focus-visible:ring-white focus-visible:ring-offset-primary-600"
        ...
        <div className="lp-reveal">
          <div className="relative isolate overflow-hidden rounded-3xl bg-gray-900 dark:bg-gray-900 border border-gray-800 px-6 py-14 sm:px-12 text-center text-white">
            <div aria-hidden="true" className="pointer-events-none absolute inset-0 -z-10">
              ...
              <div className="absolute -top-28 left-1/2 h-72 w-[560px] max-w-[140vw] -translate-x-1/2 rounded-full bg-primary-500/30 blur-3xl" />
            </div>
            ...
            <a
              href={`${ADMIN_URL}/register`}
              className={`px-6 py-3.5 mt-8 focus-visible:ring-offset-gray-900 ${btnAccent}`}
            >
              {c.cta.button}
              <ArrowRightIcon size={18} className={arrow} />
            </a>
